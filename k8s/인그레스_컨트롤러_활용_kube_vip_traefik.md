# 인그레스 컨트롤러 활용하기 (kube-vip + Traefik)
...
(kube-vip + Traefik 사용, MetalLB / ServiceLB 미사용)
...
```zsh
# 방화벽 적용 후 노드 간 Pod 통신 확인 방법 (설치 완료 후 실행)
...
curl -sfL https://get.k3s.io | INSTALL_K3S_EXEC="--disable=servicelb --tls-san=192.168.0.10" sh -

# 설치 확인
sudo systemctl status k3s
kubectl get nodes

# 워커 조인용 토큰 확인 (안전하게 보관)
...
curl -sfL https://get.k3s.io | K3S_URL="https://192.168.0.10:6443" K3S_TOKEN="<토큰값>" sh -
...
# ClusterIssuer 상태 확인 (READY: True 가 되어야 함)
...
# ⚠️ HTTP01 챌린지 전제 조건
# Let's Encrypt 서버가 외부에서 도메인의 80포트로 접근 가능해야 함.
...
9. IngressClass 확인 (Traefik, 필요 시)
...
백엔드에서 DB 서버를 붙일 때는 서비스 이름으로 지정하면 됨.
